...
export async function extractComponent(document: vscode.TextDocument, range: vscode.Range | vscode.Selection) {
  const componentName = await vscode.window.showInputBox({ value: "Component", title: "Give the extracted component a name" });
...
export function buildExtractedComponent(componentName: string, isTypescript: boolean, props: ExtractedProps[], selectedText: string) {
